...
1. 先查看“项目总览”，了解业务背景、用户路径、北极星指标和项目边界。  
2. 再体验“简历优化”和“JD 解析”，理解产品如何生成建议以及如何记录采纳行为。  
3. 最后进入“数据分析看板”，查看漏斗流失、Prompt A/B、建议质量、用户分群和产品优化优先级。  

**如何解读本项目**

本项目重点展示从产品路径到指标体系、从问题定位到策略输出的分析闭环。所有数据结论均基于模拟产品日志，用于说明分析方法和产品评估思路，不代表真实商业结果。
"""
    )
